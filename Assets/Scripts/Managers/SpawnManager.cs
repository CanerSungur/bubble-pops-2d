...
namespace BubblePops
{
    public class SpawnManager : MonoBehaviour
    {
        ...
        internal const int COLUMN_NUMBER = 5;
        ...
        private void SpawnNewColumn(Enums.ColumnLeanSide columnLeanSide)
        {
            ...
            for (int i = 0; i < ROW_NUMBER; i++)
            {
                ...
                if (columnLeanSide == Enums.ColumnLeanSide.Left)
                    bubble.transform.localPosition = new Vector2((i * ROW_OFFSET) + DOUBLE_ROW_OFFSET, -_spawnedColumnCount * COLUMN_OFFSET);
                else
                    bubble.transform.localPosition = new Vector2(i * ROW_OFFSET, -_spawnedColumnCount * COLUMN_OFFSET);

                bubble.InitAsSlotBubble(this, i, 0, columnLeanSide);
            }

            BubbleEvents.OnIncreaseBubbleColumnNumber?.Invoke();
        }
